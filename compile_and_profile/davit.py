...
class DaViT(nn.Module):
    ...
    def _try_remap_keys(self,pretrained_dict):
        remap_keys = {
            "stem.conv": "convs.0.proj",
            "stem.norm": "convs.0.norm",
            "stages_1.downsample.norm": "convs.1.norm",  
            "stages_1.downsample.conv": "convs.1.proj",
            "stages_2.downsample.norm": "convs.2.norm",
            "stages_2.downsample.conv": "convs.2.proj",
            "stages_3.downsample.norm": "convs.3.norm",
            "stages_3.downsample.conv": "convs.3.proj",
            "main_blocks": "blocks",
            "0.cpe1.proj": "spatial_block.conv1.fn.dw",
            "0.attn": "spatial_block.window_attn.fn",
            "0.cpe2.proj": "spatial_block.conv2.fn.dw",
            "0.mlp": "spatial_block.ffn.fn.net",
            "1.cpe1.proj": "channel_block.conv1.fn.dw",
            "1.attn": "channel_block.channel_attn.fn",
            "1.cpe2.proj": "channel_block.conv2.fn.dw",
        ...
        for key,value in pretrained_dict.items():
            new_key = key
            # Handle 'stages_' prefix and block indices
            if key.startswith('stages_'):  # Changed to 'stages_'
                parts = key.split('.')
                if len(parts) >= 5:  # Ensure enough parts for remapping
                    stage_index = parts[0].split('_')[1] # Extract stage index after '_'
                    block_index = parts[2]  
                    remaining_key = '.'.join(parts[3:]) 

                    # Apply remapping to the remaining key
                    for old_key, remap_key in remap_keys.items():
                        if old_key in remaining_key:
                            remaining_key = remaining_key.replace(old_key, remap_key)
                            break  # Stop replacing once a match is found

                    # Construct the new key
                    new_key = f"blocks.{stage_index}.{block_index}.{remaining_key}"
            #Directly Replace keys based on remap_keys
            for old_key, remap_key in remap_keys.items():
                if old_key in new_key: # Check for exact match
                    new_key = new_key.replace(old_key, remap_key)
                    break
            full_key_mappings[new_key] = value 
        return full_key_mappings


    def from_state_dict(self, pretrained_dict, pretrained_layers=[], verbose=True):
        model_dict = self.state_dict()
        stripped_key = lambda x: x[9:] if x.startswith('stages.') else x
        #full_key_mappings = self._try_remap_keys(pretrained_dict)
        
        #pretrained_dict = {
        #    k: v for k, v in full_key_mappings.items()
        #}
