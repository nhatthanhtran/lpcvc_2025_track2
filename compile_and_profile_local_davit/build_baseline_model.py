...
def build_baseline_model(image_input, text_input, output_path="./compile_and_profile_local_davit", test_torch_model_local=True):
    """
    ...
    #BACK BONE
    conf_file_bkbone = "./configs/xdecoder/davitd3_unicl_lang.yaml"
    opt_bkbone = load_opt_from_config_files([conf_file_bkbone])
    # #print(opt['MODEL']['BACKBONE']['DAVIT']['DIM_EMBED'])

    pretrained_path = "./lpcvc_track2_models/davit_small.msft_in1k.pth"
    ckpt_bkbone = torch.load(pretrained_path)

    # opt_bkbone = opt
    # ckpt_bkbone = ckpt
    # build backbone
    backbone = build_backbone(opt_bkbone).to(device)
    backbone.from_state_dict(ckpt_bkbone)
    # backbone.load_state_dict({k.replace('backbone.', ''): v for k,v in ckpt.items() if 'backbone' in k}, strict=True)
    # backbone = timm.create_model('davit_base.msft_in1k', pretrained=True).to(device)
    ...
    # backbone = timm.create_model('vit_base_patch16_224.augreg2_in21k_ft_in1k', pretrained=True, features_only=True,
    # ).to(device)
    ...
    pdb.set_trace() # Debugging breakpoint to check the model structure and inputs before inference
    torch.save(model.state_dict(), "./model_state_dict_davit.pt")
    if test_torch_model_local:
        inference_torch_local(model, image_input, text_input, output_path)

    return model

def inference_torch_local(model, image_input, text_input, output_path='./compile_and_profile_local_david'):
    """
...
class XDecoder(nn.Module):
    ...
    def forward(self, image_input, text_input):
        ...
        visual_features = {'res2': backbone_out[0], 'res3': backbone_out[1], 'res4': backbone_out[2], 'res5': backbone_out[3]}
